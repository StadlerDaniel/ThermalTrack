...
class Evaluator:
    ...
    @_timing.time
    def evaluate(self, dataset_list, metrics_list, return_mota=False, return_idf1=False, return_hota=False):
        ...
        for dataset, dataset_name in zip(dataset_list, dataset_names):
            ...
            for tracker in tracker_list:
                ...
                try:
                                ...
                                if config['PRINT_RESULTS'] and config['PRINT_ONLY_COMBINED']:
                                    ...
                                    if not dont_print:
                                        ...
                                        if first:
                                            ...
                                            mode = 'a'
                                        with open(os.path.join(output_fol, 'results.txt'), mode) as f:
                                            f.write(metric.write_table({'COMBINED_SEQ': table_res['COMBINED_SEQ']},
                                                           tracker_display_name, c_cls))
                                            f.write('\n')
                                elif config['PRINT_RESULTS']:
                                    metric.print_table(table_res, tracker_display_name, c_cls)
                                    if metric_name == 'HOTA':
                                        hota = float(metric._summary_row(table_res['COMBINED_SEQ'])[0])
                                    elif metric_name == 'CLEAR':
                                        mota = float(metric._summary_row(table_res['COMBINED_SEQ'])[0])
                                    elif metric_name == 'Identity':
                                        idf1 = float(metric._summary_row(table_res['COMBINED_SEQ'])[0])
                                    if first:
                                        mode = 'w'
                                    else:
                                        mode = 'a'
                                    with open(os.path.join(output_fol, 'results.txt'), mode) as f:
                                        f.write(metric.write_table(table_res, tracker_display_name, c_cls))
                                        f.write('\n')
                    ...
                    with open(os.path.join(output_fol, 'results.txt'), 'a') as f:
                        f.write('\n HOTA = {}'.format(hota))
                    print('\n HOTA = {}'.format(hota))
                    try:
                        with open(os.path.join(output_fol, 'results.txt'), 'a') as f:
