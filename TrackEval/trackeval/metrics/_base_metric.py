...
class _BaseMetric(ABC):
    ...
    def write_table(self, table_res, tracker, cls):
        """Returns printed table as string """
        string = ''
        metric_name = self.get_name()
        string += self._row_write([metric_name + ': ' + tracker + '-' + cls] + self.summary_fields)
        string += '\n'
        for seq, results in sorted(table_res.items()):
            if seq == 'COMBINED_SEQ':
                continue
            summary_res = self._summary_row(results)
            string += self._row_write([seq] + summary_res)
            string += '\n'
        summary_res = self._summary_row(table_res['COMBINED_SEQ'])
        string += self._row_write(['COMBINED'] + summary_res)
        string += '\n'
        return string
